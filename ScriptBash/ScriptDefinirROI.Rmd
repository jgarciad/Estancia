...
El objetivo de este proyecto consiste en determinar si los niveles de desigualdad de género e índices de marginalidad por municipio en México son reelevantes para la estructura cerebral dentro de un grupo de individuos de estuido.
...
Este documento se centra en el primer paso en donde se podrá ver en código bash utilizado para obtener la ROI de interés. El primer paso para realizar es descargar la máquina virtual (VM) que se encuentra en la siguiente ruta: <https://github.com/CobraLab/MINC-VM>. Para poder ejecutar la VM se pude hacer de varias formas, en nuestro caso usaremos VirtualBox el cual es de uso gratuito y se puede descargar desde la siguiente página: <https://www.virtualbox.org/wiki/Downloads>. Bata con buscar la versión según el sistema operativo que se esté utilizando.
...
Note, que en nuestro caso la ruta de interés es _/media/sf_Estancia_. Podemos explorar los archivos que hay ahí desde la terminal tecleando ls. Ahora bien, para definir la región de interés tenemos que enmascarar el cerebro, y para ello es necesario buscar la forma de identificar la región del cerebro que se quiere enmascarar. 
