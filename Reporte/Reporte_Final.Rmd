...
En los últimos años la sociedad lucha de manera incansable contra efectos de desigualdad y marginación. Cada día más personas, en especial mujeres, se suman a un movimiento para reclamar igualdad de condiciones en un entorno en donde, a pesar de que las leyes dictan dicha igualdad, no pasa así en muchas de las acciones que vemos en el día a día. De igual forma, muchas organizaciones civiles luchas por erradicar la marginación que se vive en muchas regiones no sólo de México sino en el mundo. Este comportamiento desigual que a últimas se ha logrado disminuir en cierta forma también trae consigo otro tipo de repercusiones menos comunes, pero igual de importantes como lo son los efectos a la salud.

En este ámbito, han surgido varios estudios como [@zugman_country-level_2023] en donde por país, se habla de que los niveles de desigualdad de género están asociados con diferencias estructurales en los cerebros de hombres y mujeres en donde resaltan países con mayor desigualdad de género o en estudios como [@imach2008habilidades] demuestran que la pobreza afecta la capacidad cognitiva de los niños. La marginación también puede afectar nuestras emociones pues nuestro cuerpo puede estar sujeto a contantes cambios de humor, estrés, ansiedad que incluso desde edades muy tempranas pueden presentarse.

Basados en esta idea, el objetivo del presente artículo es analizar el grosor cortical de un grupo de individuos distribuido en pacientes y sujetos sanos. El grosor cortical, como describiremos con más detalle interviene en funciones cognitivas tan importantes como la percepción, el leguaje, la memoria o la conciencia [@burgaleta2014cognitive].  



\section{Explicación del problema}

Ya hemos mencionado el grosor cortical, pero en realidad qué es. La estructura del cerebro es tan compleja que existen aparatos especializados para obtener un mapa de este desde distintos ángulos y en distintas dimensiones. Una forma de obtener este mapa del cerebro es mediante el uso de resonancias magnéticas (MRI por sus siglas en inglés) en donde con el uso de scanners especiales se obtiene información muy valiosa del cerebro que permite entender su estructura cerebral mediante imágenes y medidas de interés como lo es el grosor cortical. 

Este último, no es más que un mapa de vértices con un orden previamente establecido y donde cada vértice es la medida en milímetros de la zona del cerebro que representa. Al unir cada vértice con softwares especiales podemos obtener un mapeo del grosor cortical y sus variaciones en las distintas zonas del cerebro. Bajo dicha estructura, es de nuestro interés explicar como la medida de cada vértice se ve afectada por variables cualitativas y cuantitativas para un grupo de individuos de los cuales se ha obtenido su resonancia magnética. 

De manera específica estamos interesados en entender qué zonas de la corteza cerebral se ven afectadas por variables como lo es el Índice de Desigualdad (IDG) y el índice de Marginalidad (IM), este último publicado por el INEGI. El objetivo es entender mediante un modelo estadístico la influencia de dichas variables asociadas a cada individuo y su estructura cerebral vista desde la medida del grosor cortical. Este tipo de relaciones si bien, por si solas no pueden ayudar a diagnosticar algún problema en específico, si pueden servir como pauta para entender posibles anomalías como consecuencia de estar expuesto a altos niveles de marginalidad o de desigualdad basado en el historial clínico del paciente.
...
Uno de los modelos más comunes para explicar el comportamiento de distintos fenómenos a través de características cualitativas y cuantitativas son los modelos de regresión. En nuestro particular caso esto no es la excepción. Un modelo de regresión está representado de la siguiente manera:

$$(1)y_i = \beta_0 + \beta_1*x_{1i} + + \beta_2x_{2i} + ...+\beta_nx_{ni} + \epsilon_i, \:\:\:\:\:\:\:\:\:\: i = 1,...,N$$

En donde $y_i$ representa la variable que queremos explicar a través de características observables $x_i$ correspondientes al objeto de estudio. El objetivo está en encontrar los valores $\beta's$ tales que minimicen el resultado entre ambos lados de la ecuación. Finalmente, para que se dé una condición de igualdad, agregamos $\epsilon_i$ que representa un remanente conocido como ruido aleatorio. 

De manera más general, para n individuos, con m variables nuestro modelo queda representado de la siguiente manera:

$$(2)Y = \beta X + \epsilon$$

En donde $Y$ es un vector de $nx1$ variables, $\beta$ es un vector de $nx1$, $X$ es un vector de $n x (m+1)$ y a esto le añadimos el componente aleatorio que es un vector de $nx1$. La idea entonces es obtener los valores de $\beta$ que minimicen la diferencia entre los valores $Y$ y $\beta X$. El método más común para encontrar los valores $\beta$ es a través del método de mínimos cuadrados cuya representación matricial es la siguiente:

$$(3)\beta = (X^{T}X)^{-1}X^{T}y$$

Pensemos ahora que para cada sujeto $y_i$ tiene $p$ características distinta y para cada una de ellas nos interesa encontrar distintos valores de $\beta$ tales que se cumpla la ecuación

$$(4)y_{vi} = \beta_0 + \beta_1*x_{1i} + + \beta_2x_{2i} + ...+\beta_nx_{ni} + \epsilon_i, \:\:\:\:\:\:\:\:\:\: i = 1,...,N$$
Donde $v$ toma valores de 1 hasta $p$, esto implicaría tener $p$ modelos de regresión lineal, es decir múltiples regresiones lineales y para los cuales vamos a encontrar distintos valores de $\beta$. El método para obtener los valores de este último es casi el mismo que en (3) pero con una ligera variante, ya que ahora y será una matriz $Y$ de $nxp$ tal que:
...
Para cada modelo de regresión lineal se mide qué tan significativa resulta ser la relación entre la variable dependiente $y$ y las variables explicativas $x$. Esto se puede traducir como qué tan probable es que el coeficiente $\beta$ tome un valor distinto de cero. Para medir eso, se realiza una prueba de hipótesis sobre cada coeficiente tal que $H_0: \beta = 0$.

Lo anterior significa hacer múltiples pruebas de hipótesis y por cuestiones prácticas es mejor evaluarlas observando el $p-valor$ asociado. La definición más práctica del $p-valor$ es aquella que lo describe como la probabilidad de obtener un valor tan extremo como la distribución de los datos bajo la hipótesis nula. Si esta probabilidad es pequeña, quiere decir que el valor observado comparado con la distribución del estadístico de prueba bajo la hipostasis nula es muy diferente, lo que nos sugiere rechazar $H_0$, en contraste, si la probabilidad es grande, es muy posible que el valor observado se derive de la distribución originada bajo $H_0$ y por ende no la rechazamos.

Dado que tomamos decisiones con múltiple $p-value$ estadísticamente es posible que tomemos falsos positivos, por lo que es necesario tener una medición que nos permita decidir, que proporción de esos falsos positivos estamos dispuesto a aceptar para sacar conclusiones confiables de nuestros resultados. Eso ha dado origen a distintas pruebas de corrección que permiten medir este fenómeno, un método muy común para hacer esto es utilizando Tasas de Falsos Positivos o False Discovery Rate (FDR) por sus siglas en ingles.
...
De hecho, para cualquier $0\leqslant m_0\leqslant m$ y $p-valores$ independientes correspondientes a $H_0$ verdadera y para cualquier valor $m_1 = m - m_0$ $p-valores$ correspondientes a $H_0$ no verdadera considerando pruebas múltiples y siguiendo (8), se satisface que:
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%", out.height="50%", fig.align='center'}
...
p_popup <- paste0("<strong>Número de pacientes: </strong>", municipos2$Total_pte)

    
map <- leaflet(municipos2) %>% addTiles()
...
La figura 2 muestra la distribución del IDG considerando todos los municipios de México. Las líneas punteadas muestran el valor mínimo, máximo y la media del IDG de los individuos dentro de nuestro estudio. La distribución del IDG tiene valores atípicos, con municipios con un IDG muy bajo, pero también otros con niveles muy altos, en nuestro caso no tenemos ninguna persona que se encuentre dentro de esos rangos de la distribución.


```{r, message=FALSE, warning=FALSE, echo=FALSE, out.width="50%", fig.align='center'}
idg_municipios <- read.csv("IDG_municipios_total.csv")
idg_municipios %>% ggplot(aes(x = IDG)) + geom_histogram(fill = "lightblue") +
...
Cada una de las variables anteriores (IDG, IM, Edad, Grupo y Genero) son parte de las variables explicativas dentro del modelo propuesto.
Por otro lado, en el caso de las variables dependientes, como ya se mencionó a cada uno de los pacientes se le ha practicado una resonancia magnética o MRI. Las imágenes del cerebro se han obtenido con dos tipos de scanner GE y Siemens y han sido sujetas a controles de calidad para garantizar la veracidad de los datos. Dichas imágenes se han creado en formato _mnc_ uno de los más usados en medicina de acuerdo con [@larobina_medical_2014]. 

De acuerdo con [@fninf] surgiendo en 1995 como una disciplina formal, la cartografía cerebral humana se ha convertido en una metodología de investigación indispensable en numerosos proyectos de investigación clínica básica que estudian diversas poblaciones utilizando neuroimagen. Los datos de neuroimagen recopilados en un estudio de cohorte típico son multi-espectrales y de multi-resolución, por lo que contienen un conjunto rico de metadatos relacionados con los parámetros de exploración o el diseño del estudio. Estos datos se integrarán en última instancia con una miríada de variables específicas del sujeto, como el comportamiento, biométricas y genéticas. Debido a que muchos de estos estudios son longitudinales y los datos se proporcionan a través de canales de múltiples fuentes, tanto el análisis en línea como el seguimiento de la procedencia son esenciales.
...
Particularmente, las imágenes se obtuvieron con ayuda del CIVET el cual es un pipeline de procesamiento de imágenes cerebrales humanas para análisis corticométricos, morfométricos y volumétricos totalmente automatizados de imágenes de resonancia magnética (RM). El pipeline está compuesto por un conjunto de scripts Perl y programas de alto nivel en C/C++ [@noauthor_bic_nodate].
...
Cada salida que hacer referencia al grosor cortical tiene un total de 40,962 vértices, cada uno de estos formará una regresión lineal en conjunto con el resto de los pacientes, es decir el primer vértice de cada paciente es procesado y se forma un primer modelo y así de manera sucesiva. En total se tendrían que realizar 40,962 regresiones con múltiples pruebas de hipótesis, este escenario es ideal para el modelo propuesto en la sección anterior y que será aplicado. 
...
En esta última zona del cerebro, entre otras cosas podemos encontrar el cerebelo que está asociado con funcionas motoras, por lo que para refinar el modelo esta parte será descartada del análisis pues no estamos relacionando nuestro estudio con este tipo de funcionalidades, es por ello que es necesario _enmascarar_ el cerebro, lo cual implica descartar los vértices adecuados. En el anexo de este documento se puede observar el procedimiento y el código utilizado para ello. La siguiente imagen muestra la región subcortical del cerebro de uno de los individuos de estudio.
...
```{r, echo=FALSE,fig.show='hold', out.width = "70%", fig.align = "center", fig.cap="Proyección IDG sobre el cerebro"}
...
Los resultados del modelo muestran justamente que mientras mayores niveles de IDG estadísticamente se tiende a tener menores niveles de grosor cortical, esto se puede observar en las escalas obtenidas en la figura 3 en donde todos los valores son negativos, esto quiere decir que mientras más elevado es el IDG menor será el grosor cortical. 

Lo mismo, pero de forma contraria pasa con los niveles de marginación, en donde se puede observar que los valores de los coeficientes son positivos, esto quiere decir que mientras menor es el valor del IM se observa un menor grosor cortical. Esto muestra sin duda, que realmente existe un efecto adverso en los niveles del grosor cortical cuando estas variables son poco o nada favorables para el individuo. La siguiente gráfica muestra los niveles promedio del grosor cortical en las zonas afectadas por cada una de las variables.

```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%", fig.align='center', fig.cap="Promedio del grosor cortical comparado contra el Ìndice de Desigualdad de Género"}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%", fig.align='center', fig.cap="Promedio del grosor cortical comparado contra el indice de marginalidad"}
...
g1 <- idg_pacientes %>% ggplot(aes(x = IDG, y = MM_left, colour = as.factor(Gender))) + 
  geom_point() +
  geom_smooth(method="lm", se=FALSE) + theme_bw() + ggtitle("Hemisferio Izquierdo") +
  ylab("") + xlab("")

g2 <- idg_pacientes  %>% ggplot(aes(x = IDG, y = IM_right, colour = as.factor(Gender))) + 
  geom_point() +
  geom_smooth(method="lm", se=FALSE) + theme_bw() + ggtitle("Hemisferio Derecho") +
  ylab("") + xlab("")
...
g1 <- idg_pacientes  %>% ggplot(aes(x = IM, y = IM_left, colour = as.factor(Gender))) + 
  geom_point() +
  geom_smooth(method="lm", se=FALSE) + theme_bw() + ggtitle("Hemisferio Izquierdo") +
  ylab("") + xlab("")

g2 <- idg_pacientes %>% ggplot(aes(x = IM, y = IM_right, colour = as.factor(Gender))) + 
  geom_point() +
  geom_smooth(method="lm", se=FALSE) + theme_bw() + ggtitle("Hemisferio Izquierdo") +
  ylab("") + xlab("")
...
Note que el caso del hemisferio derecho ambas variables muestran un efecto en el cambio estructural del grosor cortical. En el caso del IDG, los niveles se ven más afectados en el caso de los hombres, mientras que en el caso del índice de marginación las repercusiones son prácticamente iguales. El hemisferio menos afectado es el izquierdo, en donde para el caso del IDG estadísticamente no existe un cambio en el caso de las mujeres y muy ligeramente en el caso de lo hombre, mientras que en el índice de marginación la pendiente es muy similar en ambos sexos si se compara con el del hemisferio derecho. 

Lo observado hasta ahora permite concluir un par de cosas, en primer lugar, que existe evidencia significativa que muestra que claramente el valor del IDG y el IM afecta la estructura del cerebro, pero a su vez no prueba de manera contundente si afecta más a hombre o a mujeres. Esto último podría ser más concluyente si consideramos un aumento en la muestra de hombres y mujeres. Finalmente, nuestra investigación permite de alguna manera crear un antecedente considerando el entorno social de una persona en donde basado en eso, se pueda dar pie a posibles causas de anomalías que tengan que ver con la estructura cerebral.
